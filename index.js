	...
	getJSON("https://raw.githubusercontent.com/Last-Oasis-Crafter/lastoasis-crafting-calculator/master/src/items.json", function(error, response){
		for (var key in response) {
			...
			if (objetitem.includes(item)) {
				message = new Discord.MessageEmbed().setColor('#FFE400').setTitle(multiplier + "x " + response[key].name).setDescription("Here are the necessary materials");
	...
	if (!found) {
		...
		getJSON("https://raw.githubusercontent.com/dm94/lastoasisbot/master/itemsES_min.json", function(error, response){
			for (var key in response) {
				...
				let objetitem = response[key].name.toLowerCase();
				if (objetitem.includes(item)) {
					message = new Discord.MessageEmbed().setColor('#FFE400').setTitle(multiplier + "x " + response[key].name).setDescription("Aquí tienes los materiales necesarios");
					var ingredie = response[key].crafting;
					for (var i = 0; i < ingredie.length; i++) {
						let le = ingredie[i].ingredients;
						for (var ing in le) {
...
function walkerAlarm(newWalker, msg) {
	try {
		pool.query("SELECT * FROM walkers where walkerID = ?", [newWalker.walkerID], (err, result) => {
			...
			if (result != null && Object.entries(result).length > 0) {
				for (var walker in result) {
					if (!(result[walker].ownerUser === "null" || !result[walker].ownerUser) && result[walker].ownerUser != newWalker.lastUser){
